...
def make_correlation_matrix(
    ...
    if save_plot:
        plt.tight_layout()
        plt.savefig(
            plot_folder_name + file_name + "_histograms_for_sync_matrix.png", dpi=300
        )
    ...
    while p1 < len(alice_timestamps) and p2 < len(bob_timestamps):
        ...
        if abs(timestamp_difference) < coincidence_window / 2:
            measurement = str(alice_channel_bit_data[alice_timestamps[p1]]) + str(
                bob_channel_bit_data[bob_timestamps[p2]]
            )

            # Check for ZZ subspace
            if (
                alice_basis_data[alice_timestamps[p1]] == "HV"
                and bob_basis_data[bob_timestamps[p2]] == "HV"
            ):
                ZZ_subspace[measurement] += 1
            # Check for ZX subspace
            elif (
                alice_basis_data[alice_timestamps[p1]] == "HV"
                and bob_basis_data[bob_timestamps[p2]] == "AD"
            ):
                ZX_subspace[measurement] += 1
            # Check for XZ subspace
            elif (
                alice_basis_data[alice_timestamps[p1]] == "AD"
                and bob_basis_data[bob_timestamps[p2]] == "HV"
            ):
    ...
    normalized_ZZ_subspace = {
        key: val / sum(ZZ_subspace.values()) for key, val in ZZ_subspace.items()
    }
    normalized_XZ_subspace = {
        key: val / sum(XZ_subspace.values()) for key, val in XZ_subspace.items()
    }
    normalized_ZX_subspace = {
        key: val / sum(ZX_subspace.values()) for key, val in ZX_subspace.items()
    }
    normalized_XX_subspace = {
        key: val / sum(XX_subspace.values()) for key, val in XX_subspace.items()
    }

    # Find error
    normalized_ZZ_subspace_err = {
        key: val
        / sum(ZZ_subspace.values())
        * np.sqrt(
            np.sqrt(val) ** 2 / val**2
            + np.sqrt(sum(ZZ_subspace.values())) ** 2 / sum(ZZ_subspace.values()) ** 2
        )
        for key, val in ZZ_subspace.items()
    }
    normalized_XZ_subspace_err = {
        key: val
        / sum(XZ_subspace.values())
        * np.sqrt(
            np.sqrt(val) ** 2 / val**2
            + np.sqrt(sum(XZ_subspace.values())) ** 2 / sum(XZ_subspace.values()) ** 2
        )
        for key, val in XZ_subspace.items()
    }
    normalized_ZX_subspace_err = {
        key: val
        / sum(ZX_subspace.values())
        * np.sqrt(
            np.sqrt(val) ** 2 / val**2
            + np.sqrt(sum(ZX_subspace.values())) ** 2 / sum(ZX_subspace.values()) ** 2
        )
        for key, val in ZX_subspace.items()
    }
    normalized_XX_subspace_err = {
        key: val
        / sum(XX_subspace.values())
        * np.sqrt(
            np.sqrt(val) ** 2 / val**2
            + np.sqrt(sum(XX_subspace.values())) ** 2 / sum(XX_subspace.values()) ** 2
        )
        for key, val in XX_subspace.items()
    }

    exp_corr_matrix = np.array(
        [
            [
                normalized_XX_subspace["00"],
                normalized_XX_subspace["01"],
                normalized_XZ_subspace["00"],
                normalized_XZ_subspace["01"],
            ],
            [
                normalized_XX_subspace["10"],
                normalized_XX_subspace["11"],
                normalized_XZ_subspace["10"],
                normalized_XZ_subspace["11"],
            ],
            [
                normalized_ZX_subspace["00"],
                normalized_ZX_subspace["01"],
                normalized_ZZ_subspace["00"],
                normalized_ZZ_subspace["01"],
            ],
            [
                normalized_ZX_subspace["10"],
                normalized_ZX_subspace["11"],
                normalized_ZZ_subspace["10"],
                normalized_ZZ_subspace["11"],
            ],
        ]
    )
    ...
    exp_corr_matrix_text = np.array(
        [
            [
                subspace_probability_to_text(
                    normalized_XX_subspace["00"], normalized_XX_subspace_err["00"]
                ),
                subspace_probability_to_text(
                    normalized_XX_subspace["01"], normalized_XX_subspace_err["01"]
                ),
                subspace_probability_to_text(
                    normalized_XZ_subspace["00"], normalized_XZ_subspace_err["00"]
                ),
                subspace_probability_to_text(
                    normalized_XZ_subspace["01"], normalized_XZ_subspace_err["01"]
                ),
            ],
            [
                subspace_probability_to_text(
                    normalized_XX_subspace["10"], normalized_XX_subspace_err["10"]
                ),
                subspace_probability_to_text(
                    normalized_XX_subspace["11"], normalized_XX_subspace_err["11"]
                ),
                subspace_probability_to_text(
                    normalized_XZ_subspace["10"], normalized_XZ_subspace_err["10"]
                ),
                subspace_probability_to_text(
                    normalized_XZ_subspace["11"], normalized_XZ_subspace_err["11"]
                ),
            ],
            [
                subspace_probability_to_text(
                    normalized_ZX_subspace["00"], normalized_ZX_subspace_err["00"]
                ),
                subspace_probability_to_text(
                    normalized_ZX_subspace["01"], normalized_ZX_subspace_err["01"]
                ),
                subspace_probability_to_text(
                    normalized_ZZ_subspace["00"], normalized_ZZ_subspace_err["00"]
                ),
                subspace_probability_to_text(
                    normalized_ZZ_subspace["01"], normalized_ZZ_subspace_err["01"]
                ),
            ],
            [
                subspace_probability_to_text(
                    normalized_ZX_subspace["10"], normalized_ZX_subspace_err["10"]
                ),
                subspace_probability_to_text(
                    normalized_ZX_subspace["11"], normalized_ZX_subspace_err["11"]
                ),
                subspace_probability_to_text(
                    normalized_ZZ_subspace["10"], normalized_ZZ_subspace_err["10"]
                ),
                subspace_probability_to_text(
                    normalized_ZZ_subspace["11"], normalized_ZZ_subspace_err["11"]
                ),
            ],
        ]
    )
    ...
    fidelity_numerator = np.trace(
        np.matmul(exp_corr_matrix.conj().T, ideal_corr_matrix)
    ) * np.trace(np.matmul(ideal_corr_matrix.conj().T, exp_corr_matrix))
    fidelity_denom = np.trace(
        np.matmul(exp_corr_matrix.conj().T, exp_corr_matrix)
    ) * np.trace(np.matmul(ideal_corr_matrix.conj().T, ideal_corr_matrix))
    ...
    for (i, j), txt in np.ndenumerate(exp_corr_matrix_text):
        ax.text(
            ...
            ha="center",
            va="center",
            color="white" if ("0.5" in txt) or ("0.4" in txt) else "black",
        )
