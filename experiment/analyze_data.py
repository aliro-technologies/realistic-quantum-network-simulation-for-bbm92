...
def analyze_data(
    ...
    while p1 < len(alice_timestamps) and p2 < len(bob_timestamps):
        ...
        if abs(timestamp_difference) < coincidence_window/2:
            if alice_basis_data[alice_timestamps[p1]] == bob_basis_data[bob_timestamps[p2]]:
                ...
                if abs(last_coinc_diff_alice) > coincidence_window and abs(last_coinc_diff_bob) > coincidence_window:
    ...
    alice_secret_key = [alice_channel_bit_data[timestamp] for timestamp in alice_common_basis_choice_times]
    bob_secret_key = [bob_channel_bit_data[timestamp] for timestamp in bob_common_basis_choice_times]
    alice_basis_info_debug = [alice_basis_data[timestamp] for timestamp in alice_common_basis_choice_times]
    bob_basis_info_debug = [bob_basis_data[timestamp] for timestamp in bob_common_basis_choice_times]
    ...
    if key_len > 0:
        qber = 1 - (sum(1 if alice_secret_key[i] == bob_secret_key[i] else 0 for i in range(key_len)) / key_len)
        HV_err_count = (sum(1 if alice_secret_key[i] != bob_secret_key[i] and alice_basis_info_debug[i] == "HV" else 0 for i in range(key_len)))
        AD_err_count = (sum(1 if alice_secret_key[i] != bob_secret_key[i] and alice_basis_info_debug[i] == "AD" else 0 for i in range(key_len)))
    ...
    print(f"Secure key rate (bits per second): {secure_key_rate} +/- {secure_key_rate_error}")

    # Return QBER, raw key rate
    return qber, qber_error, raw_key_rate, raw_key_rate_error, secure_key_rate, secure_key_rate_error, alice_singles_counts_rate, bob_singles_counts_rate
...
if __name__ == "__main__":
    ...
    analyze_data(simulation_results_folder_path=data_folder_name,
